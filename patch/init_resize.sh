...
get_variables () {
  ...
  # added to enable specifying root partition resize in a file in /boot folder
  # size specified as integer in units g, m, k, (case insensitive, with or w/o b suffix) or s[ectors],
  ROOT_PART_RESIZE_FILE=/boot/root_partition_resize
  if [ -f $ROOT_PART_RESIZE_FILE ]; then
    ROOT_PART_RESIZE=$((`sed -e 's/g/*1024m/i' -e 's/m/*1024k/i' -e 's/k/*2/i' -e 's/[sb]//i' < $ROOT_PART_RESIZE_FILE`))
  ...
  echo "$0: ROOT PARTITION BEING EXTENDED FROM $((ROOT_PART_END - ROOT_PART_START - 1)) TO $TARGET_END SECTORS"
  sleep 5
  # end-of-change
